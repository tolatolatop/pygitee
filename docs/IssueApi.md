...
> InlineResponse20020 repos_owner_issues_number_operate_logs_get(owner, number, access_token=access_token, repo=repo, sort=sort)
...
> InlineResponse2003 repos_owner_repo_pulls_number_issues_get(owner, repo, number, access_token=access_token, page=page, per_page=per_page)
...
try:
    # 获取 Pull Request 关联的 issues
    api_response = api_instance.repos_owner_repo_pulls_number_issues_get(owner, repo, number, access_token=access_token, page=page, per_page=per_page)
    pprint(api_response)
except ApiException as e:
